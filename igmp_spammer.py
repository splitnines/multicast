...
def igmp_spam(rate, group, duration, **kwargs):
    try:
        ...
        while True:
            ...
            igmp_msg = struct.pack(
                "4sl", socket.inet_aton(group), socket.INADDR_ANY
            )

            igmp_spammer.setsockopt(
                socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, igmp_msg
            )
            print(
                f"{packet_num} {dt.datetime.now()} JOIN multicast group {group}"
            )
            packet_num += 1
            time.sleep(1 / rate)
            ...
            if kwargs["join_only"] is False:
                igmp_spammer.setsockopt(
                    socket.IPPROTO_IP, socket.IP_DROP_MEMBERSHIP, igmp_msg
                )
                print(
                    f"{packet_num} {dt.datetime.now()} LEAVE multicast group {group}"
                )
                packet_num += 1
                time.sleep(1 / rate)
